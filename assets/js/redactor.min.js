...
(function (c) {
    ...
    b.fn = c.Redactor.prototype = {keyCode: {BACKSPACE: 8, DELETE: 46, DOWN: 40, ENTER: 13, ESC: 27, TAB: 9, CTRL: 17, META: 91, LEFT: 37, LEFT_WIN: 91}, init: function (f, e) {
        this.rtePaste = false;
        ...
        this.autosaveInterval = setInterval(c.proxy(function () {
            var f = this.get();
            if (e !== f) {
                c.ajax({url: this.opts.autosave, type: "post", data: this.$source.attr("name") + "=" + escape(encodeURIComponent(f)), success: c.proxy(function (g) {
        ...
        if (this.opts.air) {
            ...
            if (!this.opts.buttonSource) {
                var f = this.opts.buttons.indexOf("html"), g = this.opts.buttons[f + 1];
        ...
        this.$editor.on("mouseup.redactor keyup.redactor", this, c.proxy(function (g) {
            ...
            if (g.type === "keyup" && g.shiftKey && j != "") {
                var f = c(this.getElement(this.getSelection().focusNode)), h = f.offset();
                h.height = f.height();
                this.airShow(h, true);
            }
        }, this));
        ...
        var j = 0, m = f.length, l = 0, e = null, g = null, p = "", h = "", o = "";
        this.cleanlevel = 0;
        for (; j < m; j++) {
            l = j;
        ...
        this.selectionSave();
        var g = this.getRange(), h = this.getElement(), f = this.getNodes();
        ...
        var f = this.getRange(), g = this.getElement(), e = this.getNodes(), h;
        if (f.collapsed || f.startContainer === f.endContainer && g) {
            e = c(g);
            h = true;
        }
        c.each(e, c.proxy(function (l, m) {
        ...
        if (m.getRangeAt && m.rangeCount) {
            ...
            g.innerHTML = f;
            var n = this.document.createDocumentFragment(), j, h;
        ...
        c.each(f, c.proxy(function (m, n) {
            if (this.opts.iframe === false && c(n).parents("div.redactor_editor").size() == 0) {
                return false;
            }
            if (typeof e === "undefined") {
        ...
        var r = c("#redactor_table_rows").val(), f = c("#redactor_table_columns").val(), n = c("<div></div>"), e = Math.floor(Math.random() * 99999), p = c('<table id="table' + e + '"><tbody></tbody></table>'), g, l, m, o;
        for (g = 0; g < r; g++) {
            l = c("<tr></tr>");
            for (m = 0; m < f; m++) {
        ...
        var e = c.proxy(function () {
            ...
            if (!this.isMobile()) {
                this.draguploadInit("#redactor_file", {url: this.opts.fileUpload, uploadFields: this.opts.uploadFields, success: c.proxy(this.fileCallback, this), error: c.proxy(function (j, h) {
                    this.callback("fileUploadError", h);
                }, this), uploadParam: this.opts.fileUploadParam});
            }
            this.uploadInit("redactor_file", {auto: true, url: this.opts.fileUpload, success: c.proxy(this.fileCallback, this), error: c.proxy(function (j, h) {
                this.callback("fileUploadError", h);
            }, this)});
        }, this);
            ...
            if (this.opts.imageUpload || this.opts.s3) {
                if (!this.isMobile() && this.opts.s3 === false) {
                    if (c("#redactor_file").length) {
                        this.draguploadInit("#redactor_file", {url: this.opts.imageUpload, uploadFields: this.opts.uploadFields, success: c.proxy(this.imageCallback, this), error: c.proxy(function (h, g) {
                            this.callback("imageUploadError", g);
                        }, this), uploadParam: this.opts.imageUploadParam});
                    }
                }
                if (this.opts.s3 === false) {
                    this.uploadInit("redactor_file", {auto: true, url: this.opts.imageUpload, success: c.proxy(this.imageCallback, this), error: c.proxy(function (h, g) {
                        this.callback("imageUploadError", g);
                    }, this)});
                } else {
        ...
        e.on("click", c.proxy(function (l) {
            ...
            var n = false, q, p, m = e.width() / e.height(), o = 20, j = 10;
        ...
        if (this.uploadOptions.input) {
            var l = "redactorUploadForm" + this.id, e = "redactorUploadFile" + this.id;
        ...
        c.ajax({url: h, dataType: "html", data: j, cache: false, contentType: false, processData: false, type: "POST", success: c.proxy(function (q) {
            q = q.replace(/^\[/, "");
            q = q.replace(/\]$/, "");
        ...
        var f = this.getParent(), g = this.getCurrent();
    ...
    c.Redactor.fn.formatLinkify = function (x, u, m, r, j) {
        var s = /(^|&lt;|\s)(www\..+?\..+?)(\s|&gt;|$)/g, q = /(^|&lt;|\s)(((https?|ftp):\/\/|mailto:).+?)(\s|&gt;|$)/g, e = /(https?:\/\/.*\.(?:png|jpg|jpeg|gif))/gi, w = /^.*(youtu.be\/|u\/\w\/|embed\/|watch\?v=|\&v=)([^#\&\?]*).*/, t = /https?:\/\/(www\.)?vimeo.com\/(\d+)($|\/)/;
        var v = (this.$editor ? this.$editor.get(0) : this).childNodes, l = v.length;
        while (l--) {
            var h = v[l];
            if (h.nodeType === 3) {
                var p = h.nodeValue;
                if (r && p) {
                    var o = '<iframe width="500" height="281" src="', g = '" frameborder="0" allowfullscreen></iframe>';
                    if (p.match(w)) {
